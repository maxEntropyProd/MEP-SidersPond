c 9-Oct-2016.  Code was added to allow passing of the MPI
c file handle for error output.  Search for jjv.
c Module errorOut is used to pass the file handle 
c generanted by MPI_FILE_OPEN for the error file.      
      module mpiErrOut
         implicit none
c        file handle for error output
         integer mpiFHerr 
      end module mpiErrOut      
...
c
c-----------------------------------------------------------------------
c       jjv: added unit for writing errors and MPI routines, etc
        use mpi
        use mpiErrOut, only: mpiFHerr
        implicit none
c-----------------------------------------------------------------------
...
c
c-----------------------------------------------------------------------
c jjv There locals are used for MPI writes to error file.
...
c-----------------------------------------------------------------------
c jjv The write outputs have been converted to MPI sequential writes
  600 continue
      write(strOut,9999) 'ERROR: BACOLI runtime error in time stepping.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) '       An error code and message should have'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) '       been issued by DASSL.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr) 
      return
  610 continue
      if (istart .eq. 1) then
         write(strOut,9998) 'ERROR: BACOLI has remeshed ', maxrsh
     ...
     &   len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr) 
      ...
      return
  620 continue
      write(strOut,9999) 'ERROR: A singular matrix arises at the initial
     &'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) '       step. '
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -42
      return
  630 continue
      write(strOut,9999) 'ERROR: A singular matrix arises during remeshi
     &ng'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9997) '       at t0 =', rpar(ipar(idasr)-1+4)
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -43
      return
  640 continue
      if (istart .eq. 1) then
         write(strOut,9998) 'ERROR: nint >', nintmx, ' at'
     &                 , ' t0 =', rpar(ipar(idasr)-1+4)
         call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     &   len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)
      else
...
c-----------------------------------------------------------------------
  ...
  710 continue
      write(strOut,9999) 'ERROR: BACOLI input violation.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require:  0 <= mflag(i) <= 1, i = 1, 2, ..., 8
     &.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -51
  ...
  711 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require:  0 <= mflag(9) <= 3'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -51
      return
  715 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require:  if mflag(4) = 1, ipar(8) must be set
     &to'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'be a positive integer.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -52
      return
  720 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require:  if mflag(6) = 1, tout must be in fro
     &nt'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF,
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'of t0.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -53
      return
  725 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require:  if mflag(6) = 1, rpar(2) must be the
     &'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'initial stepsize, thus nonzero.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -54
      return
  730 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require: npde > 0.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -55
      return
  740 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require: 2 < kcol <=', mxkcol
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -56
      return
  760 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require: x(1) < x(2) < ... < x(nint+1).'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -58
      return
  770 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require: lrp >= ', ipar(irpstp), '.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -59
      return
  780 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'Require: lip >= ', ipar(iipstp), '.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -60
      return
  790 continue
      ...
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'IDID .ne. 1, on a continuation call of BACOLI'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'If IDID > 1, set idid = 1 and tout (t0 < tout)
     &'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) 'If IDID < -1, the code cannot be continued due
     &to'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      write(strOut,9999) '              a previous error.'
      call MPI_FILE_WRITE_SHARED(mpiFHerr, trim(strOut)//CRLF, 
     & len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)    
      idid = -61
      return
...
c jjv This loop consumes 87% of execution time when the function f is 
c jjv comutationally intense.  However, it can be easily parallelized
c jjv provided the user subroutine f is thread safe.
c jjv Note, the number of concurrent threads that can be used here is
c jjv nint*kcol, which could be fairly large!
!$OMP PARALLEL DO private(i,j,ii,jj,mm,kk,work) collapse(2)
     & schedule(dynamic)
      do 30 i = 1, nint

c        ii is the value of ileft for the current collocation point.
c jjv         ii = kcol + nconti + (i - 1) * kcol

         do 20 j = 1, kcol
c jjv ii can me moved into the inner loop w/o effecting results, producing
c jjv a perjectly nested loop that can be collapsed with openMP            
            ii = kcol + nconti + (i - 1) * kcol
