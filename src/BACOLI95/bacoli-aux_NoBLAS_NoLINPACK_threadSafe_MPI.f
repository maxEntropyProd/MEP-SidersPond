c 1 Aug 2017
c This version modifies SUBROUTINE XERPRN so that outputs are
c written using MPI_FILE_WRITE_SHARED, but the calling routine
c must open the file using MPI_FILE_OPEN and pass the file
c handle using XSETUA
...
C
      ...
      DATA DAMP/0.75D0/
      !$OMP threadprivate (MAXIT, MJAC, DAMP)
C
C
C---------------------------------------------------
C     BLOCK 1.
...
c     jjv: use MPI for writing to an error file.
      use mpi
c-----------------------------------------------------------------------
c     Explicit declarations made for variables declared by the implicit
c     Fortran 77 conventions.
...
C
      IF (LENMSG .EQ. 0) THEN
         ...
         DO 40 I=1,NUNIT
            WRITE(strOut, '(A)') CBUFF(1:LPREF+1)
            call MPI_FILE_WRITE_SHARED(IU(I), trim(strOut)//CRLF, 
...
C
      DO 60 I=1,NUNIT
         WRITE(strOut, '(A)') CBUFF(1:LPREF+LPIECE)
         call MPI_FILE_WRITE_SHARED(IU(I), trim(strOut)//CRLF, 
     &   len_trim(strOut)+2, MPI_CHARACTER, MPI_STATUS_IGNORE, mpiErr)
   60 CONTINUE
C
...
C-----------------------------------------------------------------------
      ...
      !$OMP threadprivate (J, DELTAM, DELTAP)
